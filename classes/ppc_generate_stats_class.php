...
class PPC_generate_stats {
    ...
    static function produce_stats( $time_start, $time_end, $author = NULL, $format = true ) {
        ...
        if( $cached_stats = PPC_cache_functions::get_stats_snapshot( $time_start, $time_end, $author ) )
    ...
    static function get_requested_posts( $time_start, $time_end, $author = NULL ) {
        ...
        self::$grp_args = array(
            ...
            'post_status' => array( 'publish', 'pending', 'future', 'private' ), // allowed post status can vary per user, so we allow them all in query and filter in data2cash
            'date_query' => array(
