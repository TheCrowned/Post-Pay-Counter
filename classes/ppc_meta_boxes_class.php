...
class PPC_meta_boxes {
    ...
    static function meta_box_misc_settings( $post, $current_settings ) {
        ...
        echo PPC_HTML_functions::echo_p_field( __( 'Display all users in stats' , 'post-pay-counter'), $current_settings['stats_show_all_users'], 'checkbox', 'stats_show_all_users', __( 'Always show all users in stats, even if they do not have written posts in the given time range. Useful if you have author payment criteria to users who do not publish posts.', 'post-pay-counter'), NULL, 'stats_show_all_users' );
        echo '</div>';
        echo '</div>';
    ...
    static function meta_box_counting_settings( $post, $current_settings ) {
        ...
        echo PPC_HTML_functions::echo_text_field( 'basic_payment_value', $current_settings['basic_payment_value'], __( 'Basic payment fixed value' , 'post-pay-counter'), 2 );
        ...
        echo '<div class="ppc_title">'.__( 'Counting options' , 'post-pay-counter').'</div>';
        echo PPC_HTML_functions::echo_text_field( 'counting_words_threshold_max', $current_settings['counting_words_threshold_max'], __( 'Stop counting words after word # (0 = infinite)' , 'post-pay-counter'), 2 );
        ...
        echo PPC_HTML_functions::echo_p_field( __( 'Include post excerpt in words counting', 'post-pay-counter' ), $current_settings['counting_words_include_excerpt'], 'checkbox', 'counting_words_include_excerpt', __( 'Determines whether post excerpt text should be included in words counting.' , 'post-pay-counter') );
        ...
        echo PPC_HTML_functions::echo_p_field( sprintf( __( 'Exclude %1$spre%2$s tags from word counting', 'post-pay-counter' ), '<em>', '</em>' ), $current_settings['counting_words_exclude_pre'], 'checkbox', 'counting_words_exclude_pre', sprintf( __( 'All the words contained into %1$spre%2$s tags will not be taken into account when counting. Notice that words included in any HTML tag with class %1$sppc_exclude_words%2$s are automatically excluded from word counting (doesn\'t handle nested tags); see FAQ for more information.', 'post-pay-counter'), '<em>', '</em>' ) );
        echo PPC_HTML_functions::echo_p_field( __( 'Exclude figure captions from word counting', 'post-pay-counter' ), $current_settings['counting_words_exclude_captions'], 'checkbox', 'counting_words_exclude_captions', __( 'Words contained into figure captions will not be taken into account.', 'post-pay-counter') );
        echo PPC_HTML_functions::echo_p_field( __( 'Apply shortcodes', 'post-pay-counter' ), $current_settings['counting_words_apply_shortcodes'], 'checkbox', 'counting_words_apply_shortcodes', __( 'Shortcodes will be applied before counting words.', 'post-pay-counter') );
        echo PPC_HTML_functions::echo_p_field( __( 'Use legacy word counter', 'post-pay-counter' ), $current_settings['counting_words_legacy'], 'checkbox', 'counting_words_legacy', __( 'Keep using word counter from before version 2.770. This has a negative impact on performance. The new word counter is 10% faster, but may have slight differences in number of words.' , 'post-pay-counter') );
        echo PPC_HTML_functions::echo_p_field( __( 'Make super cautious space parsing when computing word count' , 'post-pay-counter'), $current_settings['counting_words_parse_spaces'], 'checkbox', 'counting_words_parse_spaces', __( 'This does an extra, super-cautious parsing of blank spaces when computing word count for word payment. This is only compatible with the legacy counter, and is usually unnedeed (turns all kind of blank spaces into normal spaces).', 'post-pay-counter'), NULL, 'counting_words_parse_spaces' );
        echo PPC_HTML_functions::echo_text_field( 'counting_words_global_threshold', $current_settings['counting_words_global_threshold'], sprintf( __( 'Require at least # %s to qualify for (any) payment', 'post-pay-counter'), __( 'words', 'post-pay-counter' ) ), 2 );
        echo '</div>';
        echo '</div>';
        ...
        echo '<div class="ppc_title">'.__( 'Counting options' , 'post-pay-counter').'</div>';
        echo PPC_HTML_functions::echo_text_field( 'counting_images_threshold_min', $current_settings['counting_images_threshold_min'], __( 'Start paying per image after image #' , 'post-pay-counter'), 2 );
        echo PPC_HTML_functions::echo_text_field( 'counting_images_threshold_max', $current_settings['counting_images_threshold_max'], __( 'Stop paying per image after image #' , 'post-pay-counter'), 2 );
		...
		echo PPC_HTML_functions::echo_text_field( 'counting_images_global_threshold', $current_settings['counting_images_global_threshold'], sprintf( __( 'Require at least # %s to qualify for (any) payment', 'post-pay-counter'), __( 'images', 'post-pay-counter' ) ), 2 );
        ...
        echo '<div class="ppc_title">'.__( 'Counting options' , 'post-pay-counter').'</div>';
        echo PPC_HTML_functions::echo_text_field( 'counting_comments_threshold_min', $current_settings['counting_comments_threshold_min'], __( 'Start paying per comment after comment #' , 'post-pay-counter'), 2 );
        echo PPC_HTML_functions::echo_text_field( 'counting_comments_threshold_max', $current_settings['counting_comments_threshold_max'], __( 'Stop paying per comment after comment #' , 'post-pay-counter'), 2 );
        echo PPC_HTML_functions::echo_text_field( 'counting_comments_global_threshold', $current_settings['counting_comments_global_threshold'], sprintf( __( 'Require at least # %s to qualify for (any) payment', 'post-pay-counter'), __( 'comments', 'post-pay-counter' ) ), 2 );
        ...
        echo '<div class="main">';
        echo PPC_HTML_functions::echo_text_field( 'counting_payment_total_threshold', $current_settings['counting_payment_total_threshold'], __( 'Set payment maximum (0 = infinite)' , 'post-pay-counter'), 2 );
    ...
    */
    ...
    static function meta_box_error_log( $post, $current_settings ) {
        ...
        if( $errors !== false and ! empty( maybe_unserialize( $errors ) )) {
